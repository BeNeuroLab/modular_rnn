...
class ODEModule(nn.Module):
    ...
    def sample_random_hidden_state_vector(self) -> torch.Tensor:
        return .1 + .01 * torch.randn(self.n_dims).to(self.device)

    def sample_random_hidden_state_batch(self) -> torch.Tensor:
        return .1 + .01 * torch.randn(1, self.batch_size, self.n_dims).to(self.device)
    ...
    def f_step(self) -> None:
        ...
        if self.noisy:
            x += self.alpha * self.noise_amp * torch.randn(1, self.batch_size, self.n_dims).to(self.device)
